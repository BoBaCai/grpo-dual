...
# -*- coding: utf-8 -*-
"""
Temperature vs Entropy 测试 (手动指定路径版)

直接在notebook中运行，无需路径检测
"""
...
print("="*80)
print(f"\ngrpo-dual目录: {GRPO_DUAL_DIR}")

# 验证路径
src_dir = GRPO_DUAL_DIR / 'src'
if not (src_dir / 'grpo' / 'trainer.py').exists():
    print(f"❌ 找不到trainer.py: {src_dir / 'grpo' / 'trainer.py'}")
    print(f"\n请修改脚本中的 GRPO_DUAL_DIR 变量为正确路径")
    sys.exit(1)

print(f"✓ 找到trainer.py\n")
